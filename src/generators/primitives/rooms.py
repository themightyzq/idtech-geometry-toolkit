...
class Tower(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        # Corner walls (seal octagon corners) - thick overlap
        # Extend from oz-t to oz+total_h+t so corners seal flush with floor and ceiling
        brushes.append(self._box(ox - r - t, oy + inner - t, oz - t, ox - inner + t, oy + r + t, oz + total_h + t))
        brushes.append(self._box(ox + inner - t, oy + inner - t, oz - t, ox + r + t, oy + r + t, oz + total_h + t))
...
class Stronghold(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        # Roof ceiling slab — seals the top of the building for BSP.
        # Required regardless of battlement setting. Without this, the
        # battlement parapets only cover the perimeter, leaving the
        # interior roof open to void (causing qbsp leaks).
        brushes.append(self._box(
            ox - hw - t, oy - t, roof_z,
            ox + hw + t, oy + nl + t, roof_z + 8
        ))
...
class Courtyard(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        # === PERIMETER WALLS ===
        # Walls must extend to full ceiling height (nh) for BSP sealing.
        # The low wall_height (wh) is a visual feature only — battlements and
        # crenellation sit on top of the low wall, but the structural sealing
        # walls must reach the sky ceiling to prevent leaks.
        # Left wall
        brushes.append(self._box(ox - hw - t, oy - t, oz, ox - hw, oy + nl + t, oz + nh))
        # Right wall
        brushes.append(self._box(ox + hw, oy - t, oz, ox + hw + t, oy + nl + t, oz + nh))
...
class Antechamber(GeometricPrimitive):
    ...
    _entrance_x_offset: float = 0.0
    _exit_x_offset: float = 0.0
    _side_y_offset: float = 0.0
    ...
    def generate(self) -> List[Brush]:
        ...
        # Left (West) wall
        if self.has_side_west:
            side_y = oy + nl / 2 + self._side_y_offset
            brushes.append(self._wall_box(
                ox - hw - t, oy - t, oz,
        ...
        if self.has_exit:
            self._register_portal_tag(
                portal_id="exit",
                center_x=ox + self._exit_x_offset,
                center_y=oy + nl,
                center_z=oz,
                direction=PortalDirection.NORTH,
            )
        if self.has_side_east:
            self._register_portal_tag(
                portal_id="side_east",
                center_x=ox + hw,
                center_y=oy + nl / 2 + self._side_y_offset,
                center_z=oz,
                direction=PortalDirection.EAST,
            )
        if self.has_side_west:
...
class Amphitheater(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        if self.has_entrance:
            ...
            # Sill below portal: seal the tier area below the portal opening
            if center_floor_z < oz:
                brushes.append(self._box(
                    entrance_x - pw / 2, oy - t, center_floor_z,
                    entrance_x + pw / 2, oy, oz,
                    texture=self.texture_wall
                ))
        else:
            brushes.append(self._box(
...
class CatwalkChamber(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        if self.has_entrance:
            ...
            # Sill below portal: seal the pit face below the portal opening
            if pit_floor_z < oz:
                brushes.append(self._box(entrance_x - pw / 2, oy - t, pit_floor_z, entrance_x + pw / 2, oy, oz))
        else:
...
class SunkenChamber(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        if self.has_entrance:
            ...
            brushes.append(self._box(entrance_x - pw / 2, oy - t, oz + ph, entrance_x + pw / 2, oy, oz + nh))
            # Sill below portal: seal the basin face below the portal opening
            if basin_z < oz:
                brushes.append(self._box(entrance_x - pw / 2, oy - t, basin_z, entrance_x + pw / 2, oy, oz))
        else:
...
class LibraryArchive(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        # === SHELF ALCOVES or PLAIN WALLS ===
        if alc_d > 0:
            # Generate shelf alcoves extending into walls
            for side in [-1, 1]:
                wall_x = ox + side * hw
            ...
            # No alcoves: generate plain side walls (full height)
            brushes.append(self._box(
                ox - hw - t, oy - t, oz, ox - hw, oy + nl + t, oz + nh,
                texture=self.texture_wall
            ))
            brushes.append(self._box(
                ox + hw, oy - t, oz, ox + hw + t, oy + nl + t, oz + nh,
                texture=self.texture_wall
            ))
...
class Grotto(GeometricPrimitive):
    ...
    def generate(self) -> List[Brush]:
        ...
        if self.has_entrance:
            ...
            brushes.append(self._box(entrance_x - pw / 2, oy - t, oz + ph, entrance_x + pw / 2, oy, oz + nh))
            # Sill below portal: seal the cave floor below the portal opening
            if floor_var > 0:
                brushes.append(self._box(entrance_x - pw / 2, oy - t, oz - floor_var, entrance_x + pw / 2, oy, oz))
        else:
...
class Gatehouse(GeometricPrimitive):
    """Fortified chokepoint room with internal dividing wall.

    Features:
    - Internal wall at midpoint divides room into two halves
    - Narrow centered passage forces single-file movement
    - Lintel above passage
    - 100% sealed geometry

    Navigation pattern: CHOKE/FUNNEL — forces passage through narrow opening.
    """

    width: float = 176.0        # Half-width (3x3 = 384, hw = 384/2 - 16 = 176)
    length: float = 352.0       # Length (3x3 = 384 - 32 = 352)
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4
    passage_width: float = 48.0  # Width of passage through dividing wall
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 176.0, "min": 96, "max": 256, "label": "Half-Width",
                "description": "Half-width of the gatehouse room"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 192, "max": 512, "label": "Length",
                "description": "Total length from entrance to back wall"
            },
            "height": {
                "type": "float", "default": 128.0, "min": 96, "max": 192, "label": "Ceiling Height",
            ...
            "shell_sides": {
                "type": "int", "default": 4, "min": 3, "max": 16, "label": "Shell Sides",
                "description": "Number of sides for room shape (4=square)"
            },
            "passage_width": {
                "type": "float", "default": 48.0, "min": 32, "max": 96, "label": "Passage Width",
                "description": "Width of opening through dividing wall (player is 32 wide)"
            },
            "has_entrance": {
                "type": "bool", "default": True, "label": "Enable Entrance Portal",
                "description": "Create an opening in the front wall for entry"
    ...
    def generate(self) -> List[Brush]:
        ...
        # === INTERIOR: DIVIDING WALL WITH PASSAGE ===
        mid_y = oy + nl / 2
        wall_depth = 16.0  # Dividing wall thickness

        # Left section of dividing wall
        brushes.append(self._box(
            ox - hw, mid_y - wall_depth / 2, oz,
            ox - pw_half, mid_y + wall_depth / 2, oz + nh
        ))
        # Right section of dividing wall
        brushes.append(self._box(
            ox + pw_half, mid_y - wall_depth / 2, oz,
            ox + hw, mid_y + wall_depth / 2, oz + nh
        ))
        # Lintel above passage
        brushes.append(self._box(
            ox - pw_half, mid_y - wall_depth / 2, oz + PORTAL_HEIGHT,
            ox + pw_half, mid_y + wall_depth / 2, oz + nh
        ))
...
class Sewer(GeometricPrimitive):
    """Bifurcated passage with central drainage channel.

    Features:
    - Central trench running full length of room
    - Raised walkways on both sides
    - Optional bridge across channel at midpoint
    - 100% sealed geometry

    Navigation pattern: BIFURCATED — central trench splits room into two walkways.
    """

    width: float = 176.0        # Half-width
    length: float = 352.0       # Length
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4
    channel_depth: float = 48.0  # Depth of central channel below walkway
    channel_width: float = 64.0  # Width of central channel
    has_bridge: bool = True      # Bridge across channel at midpoint
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 176.0, "min": 96, "max": 256, "label": "Half-Width",
                "description": "Half-width of the sewer room"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 192, "max": 512, "label": "Length",
                "description": "Total length from entrance to back wall"
            ...
            "height": {
                "type": "float", "default": 128.0, "min": 96, "max": 192, "label": "Ceiling Height",
                "description": "Height from walkway floor to ceiling"
            },
            "shell_sides": {
                "type": "int", "default": 4, "min": 3, "max": 16, "label": "Shell Sides",
                "description": "Number of sides for room shape (4=square)"
            ...
            "channel_depth": {
                "type": "float", "default": 48.0, "min": 24, "max": 96, "label": "Channel Depth",
                "description": "Depth of central drainage channel below walkway"
            },
            "channel_width": {
                "type": "float", "default": 64.0, "min": 32, "max": 128, "label": "Channel Width",
                "description": "Width of central drainage channel"
            },
            "has_bridge": {
                "type": "bool", "default": True, "label": "Bridge",
                "description": "Add a bridge across the channel at midpoint"
            },
            "has_entrance": {
                "type": "bool", "default": True, "label": "Enable Entrance Portal",
                "description": "Create an opening in the front wall for entry"
    ...
    def generate(self) -> List[Brush]:
        ...
        nl = self.length
        nh = self.height
        cd = self.channel_depth
        cw_half = self.channel_width / 2

        # === WALKWAY FLOORS (left and right of channel) ===
        # Left walkway
        brushes.append(self._box(
            ox - hw - t, oy - t, oz - t,
            ox - cw_half, oy + nl + t, oz
        ))
        # Right walkway
        brushes.append(self._box(
            ox + cw_half, oy - t, oz - t,
            ox + hw + t, oy + nl + t, oz
        ))

        # === CHANNEL FLOOR (lower) ===
        brushes.append(self._box(
            ox - cw_half, oy - t, oz - cd - t,
            ox + cw_half, oy + nl + t, oz - cd
        ))

        # === CHANNEL WALLS (vertical sides of trench) ===
        brushes.append(self._box(
            ox - cw_half - t, oy - t, oz - cd,
            ox - cw_half, oy + nl + t, oz
        ))
        brushes.append(self._box(
            ox + cw_half, oy - t, oz - cd,
            ox + cw_half + t, oy + nl + t, oz
        ))

        # === CHANNEL END WALLS (seal the trench at both ends) ===
        brushes.append(self._box(
            ox - cw_half, oy - t, oz - cd,
            ox + cw_half, oy, oz
        ))
        brushes.append(self._box(
            ox - cw_half, oy + nl, oz - cd,
            ox + cw_half, oy + nl + t, oz
        ))
        ...
        if self.has_bridge:
            ...
            bridge_h = 8.0
            brushes.append(self._box(
                ox - cw_half, mid_y - bridge_w, oz,
                ox + cw_half, mid_y + bridge_w, oz + bridge_h
            ))
...
class Ossuary(GeometricPrimitive):
    """Narrow bone-storage passage with rhythmic wall niches.

    Features:
    - Shelf niches on both long walls between pilasters
    - Linear forced-forward movement pattern
    - Configurable niche count and rows
    - 100% sealed geometry

    Navigation pattern: LINEAR PASSAGE — rhythmic wall niches force forward movement.
    """

    width: float = 112.0        # Half-width (2x3 = 256x384, hw = 256/2 - 16 = 112)
    length: float = 352.0       # Length (3 cells = 384 - 32 = 352)
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4
    niche_count: int = 4         # Number of niches per side
    niche_depth: float = 24.0    # How far niches protrude inward
    niche_rows: int = 2          # Vertical rows of shelves per niche
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 112.0, "min": 64, "max": 192, "label": "Half-Width",
                "description": "Half-width of the ossuary passage"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 192, "max": 512, "label": "Length",
                "description": "Total length from entrance to back wall"
            ...
            "niche_count": {
                "type": "int", "default": 4, "min": 2, "max": 8, "label": "Niche Count",
                "description": "Number of shelf niches per wall side"
            },
            "niche_depth": {
                "type": "float", "default": 24.0, "min": 16, "max": 48, "label": "Niche Depth",
                "description": "Depth of shelf niches protruding inward"
            },
            "niche_rows": {
                "type": "int", "default": 2, "min": 1, "max": 3, "label": "Niche Rows",
                "description": "Vertical rows of shelves in each niche"
            },
            "has_entrance": {
                "type": "bool", "default": True, "label": "Enable Entrance Portal",
                "description": "Create an opening in the front wall for entry"
    ...
    def generate(self) -> List[Brush]:
        ...
        # === INTERIOR: SHELF NICHES ON BOTH WALLS ===
        usable_length = nl - 32  # Leave margins at front and back
        niche_spacing = usable_length / self.niche_count
        pilaster_width = 8.0
        niche_width = niche_spacing - pilaster_width
        shelf_thickness = 8.0
        niche_height = (nh - 16) / max(self.niche_rows, 1)

        for i in range(self.niche_count):
            niche_y_start = oy + 16 + i * niche_spacing + pilaster_width / 2
            niche_y_end = niche_y_start + niche_width

            # Pilasters between niches
            if i > 0:
                pilaster_y = oy + 16 + i * niche_spacing - pilaster_width / 2
                brushes.append(self._box(
                    ox - hw, pilaster_y, oz,
                    ox - hw + self.niche_depth, pilaster_y + pilaster_width, oz + nh
                ))
                brushes.append(self._box(
                    ox + hw - self.niche_depth, pilaster_y, oz,
                    ox + hw, pilaster_y + pilaster_width, oz + nh
                ))

            # Shelf brushes for each row
            for row in range(self.niche_rows):
                shelf_z = oz + 16 + row * niche_height

                brushes.append(self._box(
                    ox - hw, niche_y_start, shelf_z,
                    ox - hw + self.niche_depth, niche_y_end, shelf_z + shelf_thickness
                ))
                brushes.append(self._box(
                    ox + hw - self.niche_depth, niche_y_start, shelf_z,
                    ox + hw, niche_y_end, shelf_z + shelf_thickness
                ))
...
class Cloister(GeometricPrimitive):
    """Perimeter arcade room with inner pillar ring.

    Features:
    - Ring of pillars inset from walls creating inner/outer zones
    - Optional sunken center area
    - Configurable pillar count and style
    - 100% sealed geometry

    Navigation pattern: PERIMETER ARCADE — pillar ring creates inner/outer movement zones.
    """
    ...
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4
    pillar_count: int = 8        # Total pillars in ring
    pillar_style: str = "square"
    pillar_capital: bool = False
    pillar_inset: float = 48.0   # Distance from walls to pillar ring
    pillar_width: float = 16.0   # Pillar width
    sunken_depth: float = 16.0   # Depth of optional sunken center (0 = flat)
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 176.0, "min": 96, "max": 256, "label": "Half-Width",
                "description": "Half-width of the cloister room"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 192, "max": 512, "label": "Length",
                "description": "Total length from entrance to back wall"
            ...
            "pillar_count": {
                "type": "int", "default": 8, "min": 4, "max": 16, "label": "Pillar Count",
                "description": "Total number of pillars in the arcade ring"
            },
            "pillar_style": {
                "type": "choice", "default": "square",
                "choices": ["square", "hexagonal", "octagonal", "round"],
                "label": "Pillar Style",
                "description": "Shape of the arcade pillars"
            },
            "pillar_capital": {
                "type": "bool", "default": False, "label": "Pillar Capitals",
                "description": "Add decorative capitals to pillar tops"
            },
            "pillar_inset": {
                "type": "float", "default": 48.0, "min": 32, "max": 96, "label": "Pillar Inset",
                "description": "Distance from walls to pillar ring"
            },
            "pillar_width": {
                "type": "float", "default": 16.0, "min": 8, "max": 32, "label": "Pillar Width",
                "description": "Width/diameter of each pillar"
            },
            "sunken_depth": {
                "type": "float", "default": 16.0, "min": 0, "max": 48, "label": "Sunken Center Depth",
                "description": "Depth of sunken center area (0 = flat floor)"
            },
            "has_entrance": {
                "type": "bool", "default": True, "label": "Enable Entrance Portal",
                "description": "Create an opening in the front wall for entry"
    ...
    def generate(self) -> List[Brush]:
        ...
        nl = self.length
        nh = self.height
        pi = self.pillar_inset

        # === FLOOR ===
        if self.sunken_depth > 0:
            # Perimeter floor (outer ring)
            brushes.append(self._box(
                ox - hw - t, oy - t, oz - t,
                ox + hw + t, oy + pi, oz
            ))
            brushes.append(self._box(
                ox - hw - t, oy + nl - pi, oz - t,
                ox + hw + t, oy + nl + t, oz
            ))
            brushes.append(self._box(
                ox - hw - t, oy + pi, oz - t,
                ox - hw + pi, oy + nl - pi, oz
            ))
            brushes.append(self._box(
                ox + hw - pi, oy + pi, oz - t,
                ox + hw + t, oy + nl - pi, oz
            ...
            # Sunken center floor
            brushes.append(self._box(
                ox - hw + pi, oy + pi, oz - self.sunken_depth - t,
                ox + hw - pi, oy + nl - pi, oz - self.sunken_depth
            ))
            # Sunken center walls (4 sides)
            brushes.append(self._box(
                ox - hw + pi - t, oy + pi, oz - self.sunken_depth,
                ox - hw + pi, oy + nl - pi, oz
            ))
            brushes.append(self._box(
                ox + hw - pi, oy + pi, oz - self.sunken_depth,
                ox + hw - pi + t, oy + nl - pi, oz
            ))
            brushes.append(self._box(
                ox - hw + pi, oy + pi - t, oz - self.sunken_depth,
                ox + hw - pi, oy + pi, oz
            ))
            brushes.append(self._box(
                ox - hw + pi, oy + nl - pi, oz - self.sunken_depth,
                ox + hw - pi, oy + nl - pi + t, oz
            ))
        else:
            brushes.append(self._box(
                ox - hw - t, oy - t, oz - t,
        ...
        # === INTERIOR: PILLAR RING ===
        inner_x1 = ox - hw + pi
        inner_x2 = ox + hw - pi
        inner_y1 = oy + pi
        inner_y2 = oy + nl - pi

        # 4 corners always placed, extras distributed along sides
        pillar_positions = [
            (inner_x1, inner_y1), (inner_x2, inner_y1),
            (inner_x1, inner_y2), (inner_x2, inner_y2),
        ]

        remaining = max(0, self.pillar_count - 4)
        if remaining > 0:
            side_x = inner_x2 - inner_x1
            side_y = inner_y2 - inner_y1
            total_perimeter = 2 * (side_x + side_y)
            per_side_x = max(0, round(remaining * side_x / total_perimeter))
            per_side_y = max(0, round(remaining * side_y / total_perimeter))
            allocated = 2 * per_side_x + 2 * per_side_y
            while allocated < remaining:
                per_side_y += 1
                allocated += 2
            while allocated > remaining and per_side_x > 0:
                per_side_x -= 1
                allocated -= 2

            for j in range(per_side_x):
                frac = (j + 1) / (per_side_x + 1)
                px = inner_x1 + frac * (inner_x2 - inner_x1)
                pillar_positions.append((px, inner_y1))
                pillar_positions.append((px, inner_y2))

            for j in range(per_side_y):
                frac = (j + 1) / (per_side_y + 1)
                py = inner_y1 + frac * (inner_y2 - inner_y1)
                pillar_positions.append((inner_x1, py))
                pillar_positions.append((inner_x2, py))

        for px, py in pillar_positions:
            brushes.extend(self._generate_room_pillar(
                px, py, oz, oz + nh,
                self.pillar_width,
                style=self.pillar_style,
                capital=self.pillar_capital,
            ))
...
class Colosseum(GeometricPrimitive):
    """Tiered arena room with stepped platforms.

    Features:
    - Raised tier platforms on left, right, and back
    - Central lower arena floor
    - Configurable tier count and height
    - 100% sealed geometry

    Navigation pattern: HIGH/LOW CHOICE — stepped tiers around lower center arena.
    """

    width: float = 240.0        # Half-width (4x4 = 512, hw = 512/2 - 16 = 240)
    length: float = 480.0       # Length (4x4 = 512 - 32 = 480)
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4
    tier_count: int = 2          # Number of tier steps
    tier_height: float = 16.0   # Height per tier step
    tier_width: float = 48.0    # Depth of each tier step
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 240.0, "min": 128, "max": 320, "label": "Half-Width",
                "description": "Half-width of the colosseum room"
            },
            "length": {
                "type": "float", "default": 480.0, "min": 256, "max": 640, "label": "Length",
                "description": "Total length from entrance to back wall"
            },
            "height": {
                "type": "float", "default": 128.0, "min": 96, "max": 192, "label": "Ceiling Height",
                "description": "Height from arena floor to ceiling"
            },
            "shell_sides": {
                "type": "int", "default": 4, "min": 3, "max": 16, "label": "Shell Sides",
                "description": "Number of sides for room shape (4=square)"
            ...
            "tier_count": {
                "type": "int", "default": 2, "min": 1, "max": 3, "label": "Tier Count",
                "description": "Number of stepped tier levels"
            },
            "tier_height": {
                "type": "float", "default": 16.0, "min": 8, "max": 32, "label": "Tier Height",
                "description": "Height of each tier step"
            },
            "tier_width": {
                "type": "float", "default": 48.0, "min": 32, "max": 96, "label": "Tier Width",
                "description": "Depth of each tier step"
            },
            "has_entrance": {
                "type": "bool", "default": True, "label": "Enable Entrance Portal",
                "description": "Create an opening in the front wall for entry"
    ...
    def generate(self) -> List[Brush]:
        ...
        # === INTERIOR: STEPPED TIERS ===
        for i in range(tc):
            tier_z = oz + (i + 1) * th
            # Left tiers
            brushes.append(self._box(
                ox - hw + i * tw, oy + 32, oz,
                ox - hw + (i + 1) * tw, oy + nl - 32, tier_z
            ))
            # Right tiers
            brushes.append(self._box(
                ox + hw - (i + 1) * tw, oy + 32, oz,
                ox + hw - i * tw, oy + nl - 32, tier_z
            ))
        ...
        for i in range(tc):
            tier_z = oz + (i + 1) * th
            brushes.append(self._box(
                ox - hw + inner_x, oy + nl - (i + 1) * tw, oz,
                ox + hw - inner_x, oy + nl - i * tw, tier_z
            ))
...
class Hub(GeometricPrimitive):
    """Central hub room with 4 cardinal portals.

    Features:
    - 3x3 footprint room with N, S, E, W portals
    - Open interior space for navigation crossroads
    - Optional center feature: pillars, raised platform, or sunken pit
    - Follows Antechamber multi-portal pattern
    - 100% sealed geometry

    Designed as a larger multi-portal junction room (room-scale alternative
    to Crossroads hall). Portals at cell centers: N(1,2), S(1,0), E(2,1), W(0,1).
    """

    width: float = 176.0        # Half-width (3 cells * 128 / 2 - 2*16 = 176)
    length: float = 352.0       # Length (3 cells * 128 - 2*16 = 352)
    height: float = 128.0       # Ceiling height
    shell_sides: int = 4        # Number of sides (4=square)
    random_seed: int = 0

    # Portal control - 4 cardinal portals
    has_entrance: bool = True       # South (front)
    has_exit: bool = True           # North (back)
    has_side_east: bool = True      # East
    has_side_west: bool = True      # West

    # Center feature
    center_feature: str = "none"    # none, pillars, platform, pit
    pillar_style: str = "square"    # square, hexagonal, octagonal, round
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        return {
            "width": {
                "type": "float", "default": 176.0, "min": 128, "max": 256, "label": "Half-Width",
                "description": "Half-width of the hub from center to walls"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 256, "max": 512, "label": "Length",
                "description": "Total length from south to north wall"
            },
            "height": {
                "type": "float", "default": 128.0, "min": 96, "max": 192, "label": "Ceiling Height",
            ...
            "center_feature": {
                ...
                "choices": ["none", "pillars", "platform", "pit"],
                "label": "Center Feature",
                "description": "Optional feature in the center of the hub"
            },
            "pillar_style": {
                "type": "choice", "default": "square",
                "choices": ["square", "hexagonal", "octagonal", "round"],
                "label": "Pillar Style",
                "description": "Style of pillars (when center_feature is 'pillars')"
            },
            "random_seed": {
                "type": "int", "default": 0, "min": 0, "max": 999999, "label": "Random Seed",
                "description": "Seed for deterministic generation (0 = random)"
    ...
    def generate(self) -> List[Brush]:
        """Generate hub room geometry.

        Construction follows the Antechamber pattern (CLAUDE.md Section 8):
        1. Floor slab with t-extension in all directions
        2. Ceiling slab with t-extension in all directions
        3. South wall (3-piece portal or solid)
        4. North wall (3-piece portal or solid)
        5. West wall (3-piece portal or solid)
        6. East wall (3-piece portal or solid)
        7. Optional center feature (pillars, platform, pit)
        8. Portal tag registration
        """
        self._reset_tags()
        ox, oy, oz = self.params.origin
        brushes: List[Brush] = []
        ...
        # Front (South) wall — outer face at oy - t, inner face at oy
        if self.has_entrance:
            entrance_x = ox + self._entrance_x_offset
            # Left piece
            brushes.append(self._wall_box(
        ...
        # Back (North) wall — outer face at oy + nl + t, inner face at oy + nl
        if self.has_exit:
            exit_x = ox + self._exit_x_offset
            # Left piece
            brushes.append(self._wall_box(
        ...
        # Left (West) wall — outer face at ox - hw - t, inner face at ox - hw
        if self.has_side_west:
            side_y = oy + nl / 2 + self._side_y_offset
            # Bottom piece (south of portal)
            brushes.append(self._wall_box(
                ox - hw - t, oy - t, oz,
                ox - hw, side_y - pw / 2, oz + nh
            ))
            # Top piece (north of portal)
            brushes.append(self._wall_box(
                ox - hw - t, side_y + pw / 2, oz,
                ox - hw, oy + nl + t, oz + nh
            ))
        ...
        # Right (East) wall — outer face at ox + hw + t, inner face at ox + hw
        if self.has_side_east:
            side_y = oy + nl / 2 + self._side_y_offset
            # Bottom piece (south of portal)
            brushes.append(self._wall_box(
        ...
        # === OPTIONAL CENTER FEATURE ===
        center_x = ox
        center_y = oy + nl / 2

        if self.center_feature == "pillars":
            # 4 pillars in a square pattern around center
            pillar_size = 24.0
            pillar_offset = 64.0  # Distance from center to each pillar
            for dx, dy in [(-1, -1), (1, -1), (-1, 1), (1, 1)]:
                px = center_x + dx * pillar_offset
                py = center_y + dy * pillar_offset
                brushes.extend(self._generate_room_pillar(
                    px, py, oz, oz + nh,
                    pillar_size, self.pillar_style
                ))

        elif self.center_feature == "platform":
            # Raised platform in the center
            plat_hw = 48.0   # Half-width of platform
            plat_h = 16.0    # Platform height
            brushes.append(self._structural_box(
                center_x - plat_hw, center_y - plat_hw, oz,
                center_x + plat_hw, center_y + plat_hw, oz + plat_h
            ))

        elif self.center_feature == "pit":
            # Sunken pit in the center (floor lowered)
            pit_hw = 48.0    # Half-width of pit
            pit_depth = 32.0  # How far below floor
            # Pit floor
            brushes.append(self._floor_box(
                center_x - pit_hw, center_y - pit_hw, oz - pit_depth - t,
                center_x + pit_hw, center_y + pit_hw, oz - pit_depth
            ))
            # Pit walls (4 sides)
            # South pit wall
            brushes.append(self._structural_box(
                center_x - pit_hw, center_y - pit_hw - t, oz - pit_depth,
                center_x + pit_hw, center_y - pit_hw, oz
            ))
            # North pit wall
            brushes.append(self._structural_box(
                center_x - pit_hw, center_y + pit_hw, oz - pit_depth,
                center_x + pit_hw, center_y + pit_hw + t, oz
            ))
            # West pit wall
            brushes.append(self._structural_box(
                center_x - pit_hw - t, center_y - pit_hw - t, oz - pit_depth,
                center_x - pit_hw, center_y + pit_hw + t, oz
            ))
            # East pit wall
            brushes.append(self._structural_box(
                center_x + pit_hw, center_y - pit_hw - t, oz - pit_depth,
                center_x + pit_hw + t, center_y + pit_hw + t, oz
            ))
...
class ThroneRoom(GeometricPrimitive):
    """Throne room with raised dais at the back.

    A single-portal destination room featuring a raised platform (dais)
    at the back third of the room, creating vertical drama and a
    focal point. The 16-unit step up creates a boss arena feel.

    Footprint: 3x3 cells (384x384 units)
    Portal: entrance SOUTH at cell (1,0)
    Interior: front 2/3 at floor level, back 1/3 raised by dais_height
    """

    width: float = 176.0          # Half-width
    length: float = 352.0         # Full Y length
    height: float = 128.0         # Ceiling height
    dais_height: float = 16.0     # Height of raised platform
    pillar_style: str = "square"  # Pillars flanking the step
    pillar_capital: bool = False
    has_pillars: bool = True      # Flanking pillars at step
    has_entrance: bool = True
    random_seed: int = 0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 176.0, "min": 64, "max": 384,
                "label": "Half-Width",
                "description": "Half-width of the room (total width = 2x this value)"
            },
            "length": {
                "type": "float", "default": 352.0, "min": 128, "max": 768,
                "label": "Length",
                "description": "Front-to-back depth of the room"
            },
            "height": {
                "type": "float", "default": 128.0, "min": 64, "max": 256,
                "label": "Height",
                "description": "Interior ceiling height"
            },
            "dais_height": {
                ...
                "label": "Dais Height",
                "description": "Height of the raised platform at the back of the room"
            },
            "pillar_style": {
                "type": "choice", "default": "square",
                "choices": ["square", "hexagonal", "octagonal", "round"],
                "label": "Pillar Style",
                "description": "Shape of the pillars flanking the dais step"
            },
            "pillar_capital": {
                "type": "bool", "default": False,
                "label": "Pillar Capitals",
                "description": "Add decorative capitals to pillars"
            },
            "has_pillars": {
                "type": "bool", "default": True,
                "label": "Flanking Pillars",
                "description": "Add pillars flanking the dais step"
            },
            "has_entrance": {
                "type": "bool", "default": True,
                "label": "Enable Entrance Portal",
                "description": "Enable the entrance portal opening"
            },
            "random_seed": {
    ...
    def generate(self) -> List[Brush]:
        ...
        # Dais starts at 2/3 of room length (back third is raised)
        dais_start = round(nl * 2.0 / 3.0 / 16) * 16  # Snap to 16-unit grid

        # === FLOOR ===
        # Main floor (front 2/3) — t-extension all 4 outer sides + overlap past step
        brushes.append(self._floor_box(
            ox - hw - t, oy - t, oz - t,
            ox + hw + t, oy + dais_start + t, oz
        ))

        # Dais floor (back 1/3) — t-extension on 3 outer sides, meets step at south
        brushes.append(self._floor_box(
            ox - hw - t, oy + dais_start, oz + dh - t,
            ox + hw + t, oy + nl + t, oz + dh
        ))

        # Step wall (vertical face between floor levels)
        brushes.append(self._structural_box(
            ox - hw - t, oy + dais_start, oz,
            ox + hw + t, oy + dais_start + t, oz + dh
        ))
        ...
        # === SIDE WALLS (span full Y-extent, Rule 2) ===
        # Left wall
        brushes.append(self._wall_box(
            ox - hw - t, oy - t, oz,
            ox - hw, oy + nl + t, oz + nh
        ...
        # === SOUTH WALL (entrance) — outer face at footprint boundary (Rule 3 / §5.1) ===
        entrance_portal = PortalSpec(enabled=self.has_entrance)
        front_brushes, _ = generate_portal_wall(
            box_func=self._wall_box,
            x1=ox - hw - t, y1=oy - t, z1=oz,
            x2=ox + hw + t, y2=oy, z2=oz + nh,
            portal_spec=entrance_portal,
            portal_axis="x",
        ...
        # === NORTH WALL (back, solid) — outer face at footprint boundary ===
        # Starts at dais floor level (dh) since dais floor covers below
        brushes.append(self._wall_box(
            ox - hw - t, oy + nl, oz + dh,
            ox + hw + t, oy + nl + t, oz + nh
        ))
        # Fill under the back wall behind the dais (from oz to oz+dh is covered by dais floor t-extension)

        # === OPTIONAL PILLARS FLANKING THE STEP ===
        if self.has_pillars:
            pillar_size = 24
            pillar_inset = 40  # Distance from wall to pillar center
            brushes.extend(self._generate_room_pillar(
                ox - hw + pillar_inset, oy + dais_start + pillar_inset / 2, oz, oz + nh,
                pillar_size, self.pillar_style, self.pillar_capital
            ))
            brushes.extend(self._generate_room_pillar(
                ox + hw - pillar_inset, oy + dais_start + pillar_inset / 2, oz, oz + nh,
...
class Vestibule(GeometricPrimitive):
    """Small buffer room between corridors and large rooms.

    A tiny through-passage that prevents rooms from opening directly
    into each other, creating pacing contrast. Offset portals force
    a slight jog through the room.

    Footprint: 2x1 cells (256x128 units)
    Portals: entrance SOUTH at cell (0,0), exit NORTH at cell (1,0)
    """

    width: float = 112.0          # Half-width
    length: float = 96.0          # Full Y length
    height: float = 128.0         # Ceiling height
    has_entrance: bool = True
    has_exit: bool = True
    random_seed: int = 0

    # Portal offsets — set by layout generator
    _entrance_x_offset: float = 0.0
    _exit_x_offset: float = 0.0
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            "width": {
                "type": "float", "default": 112.0, "min": 48, "max": 256,
                "label": "Half-Width",
                "description": "Half-width of the room (total width = 2x this value)"
            },
            "length": {
                "type": "float", "default": 96.0, "min": 64, "max": 256,
                "label": "Length",
                "description": "Front-to-back depth of the room"
            },
            "height": {
            ...
            "has_entrance": {
                "type": "bool", "default": True,
                "label": "Enable Entrance Portal",
                "description": "Enable the entrance portal opening (SOUTH)"
            },
            "has_exit": {
                "type": "bool", "default": True,
                "label": "Enable Exit Portal",
                "description": "Enable the exit portal opening (NORTH)"
            },
            "random_seed": {
                "type": "int", "default": 0, "min": 0, "max": 999999,
                "label": "Random Seed (0=random)",
    ...
    def generate(self) -> List[Brush]:
        ...
        # === FLOOR (t-extension all 4 sides) ===
        brushes.append(self._floor_box(
            ox - hw - t, oy - t, oz - t,
            ox + hw + t, oy + nl + t, oz
        ))

        # === CEILING (t-extension all 4 sides) ===
        brushes.append(self._ceiling_box(
            ox - hw - t, oy - t, oz + nh,
            ox + hw + t, oy + nl + t, oz + nh + t
        ))
        ...
        # === SOUTH WALL (entrance) — outer face at footprint boundary ===
        entrance_x = ox + self._entrance_x_offset
        entrance_portal = PortalSpec(enabled=self.has_entrance)
        front_brushes, _ = generate_portal_wall(
            box_func=self._wall_box,
        ...
        # === NORTH WALL (exit) — outer face at footprint boundary ===
        exit_x = ox + self._exit_x_offset
        exit_portal = PortalSpec(enabled=self.has_exit)
        back_brushes, _ = generate_portal_wall(
            box_func=self._wall_box,
...
class Processional(GeometricPrimitive):
    """Long processional hall with colonnade pillars.

    A through-passage room with two rows of pillars creating a
    cathedral nave feel. Entrance and exit on opposite short walls.

    Footprint: 2x4 cells (256x512 units)
    Portals: entrance SOUTH at cell (0,0), exit NORTH at cell (1,3)
    """

    width: float = 112.0          # Half-width
    length: float = 480.0         # Full Y length
    height: float = 128.0         # Ceiling height
    pillar_count: int = 3         # Number of pillar pairs
    pillar_style: str = "square"
    pillar_capital: bool = False
    has_entrance: bool = True
    has_exit: bool = True
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            ...
            "length": {
                "type": "float", "default": 480.0, "min": 256, "max": 1024,
                "label": "Length",
                "description": "Front-to-back depth of the room"
            },
            "height": {
            ...
            "pillar_count": {
                ...
                "label": "Pillar Pairs",
                "description": "Number of pillar pairs along the length"
            },
            "pillar_style": {
                "type": "choice", "default": "square",
                "choices": ["square", "hexagonal", "octagonal", "round"],
                "label": "Pillar Style",
                "description": "Shape of the colonnade pillars"
            },
            "pillar_capital": {
                "type": "bool", "default": False,
                "label": "Pillar Capitals",
    ...
    def generate(self) -> List[Brush]:
        ...
        # === COLONNADE PILLARS ===
        pillar_size = 24
        pillar_inset = 32  # Distance from wall to pillar center
        # Evenly space pillars along the length
        for i in range(self.pillar_count):
            # Position pillars evenly between entrance and exit
            pillar_y = oy + nl * (i + 1) / (self.pillar_count + 1)
            # Left row
            brushes.extend(self._generate_room_pillar(
                ox - hw + pillar_inset, pillar_y, oz, oz + nh,
                pillar_size, self.pillar_style, self.pillar_capital
            ))
            # Right row
            brushes.extend(self._generate_room_pillar(
...
class DoglegRoom(GeometricPrimitive):
    """L-shaped room that breaks line-of-sight.

    A room with solid fill in the NW quadrant creating an L-shape.
    Players enter from the south and must turn to find the east exit.
    Creates ambush tension and tactical depth.

    Footprint: 3x3 cells (384x384 units)
    Portals: entrance SOUTH at cell (1,0), exit EAST at cell (2,2)

    Layout (top-down):
        +-----+-----+-----+
        |FILL |     | exit|  Row 2 (Y=256-384): NW cell is SOLID FILL
        +-----+-----+-----+
        |     |     |     |  Row 1 (Y=128-256): all open
        +-----+-----+-----+
        |     | ent |     |  Row 0 (Y=0-128): entrance SOUTH
        +-----+-----+-----+
    """

    width: float = 176.0          # Half-width of FULL 3x3 footprint
    length: float = 352.0         # Full Y length
    height: float = 128.0         # Ceiling height
    has_entrance: bool = True
    has_exit: bool = True
    ...
    def get_parameter_schema(cls) -> Dict[str, Dict[str, Any]]:
        schema = {
            ...
            "has_exit": {
                "type": "bool", "default": True,
                "label": "Enable Exit Portal",
                "description": "Enable the exit portal opening (EAST)"
            },
            "random_seed": {
                "type": "int", "default": 0, "min": 0, "max": 999999,
                "label": "Random Seed (0=random)",
    ...
    def generate(self) -> List[Brush]:
        ...
        hw = self.width       # Half-width of entire 3x3 room
        nl = self.length      # Full length of entire 3x3 room
        nh = self.height

        # The L-shape: NW corner (cell 0, row 2) is solid fill
        # In local coords: fill occupies X from -hw to -hw + fill_width, Y from arm_split to nl
        # Fill width = 1 cell worth = 1/3 of total width (2*hw)
        fill_width = round(2 * hw / 3.0 / 16) * 16  # ~1 cell width, snapped to grid
        fill_x_max = ox - hw + fill_width  # Right edge of fill block
        arm_split = round(nl * 2.0 / 3.0 / 16) * 16  # Y boundary where fill starts (2/3 up)

        # === NW JUNCTION FILL (solid block, Rule 5) ===
        # Full Z-extent: oz-t to oz+nh+t (floor through ceiling)
        brushes.append(self._wall_box(
            ox - hw - t, oy + arm_split, oz - t,
            fill_x_max, oy + nl + t, oz + nh + t
        ))

        # === FLOOR ===
        # South section floor (full width, rows 0-1 + overlap into row 2)
        brushes.append(self._floor_box(
            ox - hw - t, oy - t, oz - t,
            ox + hw + t, oy + arm_split + t, oz
        ))
        # North-east section floor (from fill edge to east edge, row 2)
        brushes.append(self._floor_box(
            fill_x_max, oy + arm_split, oz - t,
            ox + hw + t, oy + nl + t, oz
        ))

        # === CEILING (single slab over entire 3x3 footprint) ===
        # The fill block already covers oz+nh in NW corner, so a full-span ceiling is fine
        brushes.append(self._ceiling_box(
            ox - hw - t, oy - t, oz + nh,
            ox + hw + t, oy + nl + t, oz + nh + t
        ))
        ...
        # === WEST WALL ===
        # South portion (rows 0-1, from oy-t to arm_split)
        brushes.append(self._wall_box(
            ox - hw - t, oy - t, oz,
            ox - hw, oy + arm_split + t, oz + nh
        ))
        # North portion covered by junction fill block

        # === EAST WALL (full length, Rule 2) ===
        # South section spans oy-t to arm_split+t (must cover floor slab t-extension)
        brushes.append(self._wall_box(
            ox + hw, oy - t, oz,
            ox + hw + t, oy + arm_split + t, oz + nh
        ))
        # East wall north section (row 2, from arm_split to nl+t)
        # This section has the exit portal
        exit_y = oy + arm_split + (nl - arm_split) / 2 + self._exit_y_offset
        exit_portal = PortalSpec(enabled=self.has_exit)
        east_brushes, _ = generate_portal_wall(
            box_func=self._wall_box,
            x1=ox + hw, y1=oy + arm_split, z1=oz,
            x2=ox + hw + t, y2=oy + nl + t, z2=oz + nh,
            portal_spec=exit_portal,
            portal_axis="y",
            portal_center=exit_y,
        )
        brushes.extend(east_brushes)
        ...
        # === NORTH WALL (east arm only — from fill_x_max to east edge) ===
        brushes.append(self._wall_box(
            fill_x_max, oy + nl, oz,
            ox + hw + t, oy + nl + t, oz + nh
        ))

        # === INNER WALL (south face of fill block — from fill_x_max west to fill left) ===
        # This is already sealed by the junction fill block extending down to oz-t
