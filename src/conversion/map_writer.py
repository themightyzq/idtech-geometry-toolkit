...
class MapWriter:
    ...
    def write_to_file(self, filename: str, validate: bool = True) -> None:
        ...
        # Write to a temporary file first, then atomic-rename on success.
        # This prevents partial/corrupt .map files if writing fails mid-stream.
        output_path = Path(filename)
        tmp_path = output_path.with_suffix('.map.tmp')
        ...
        try:
            ...
            logger.info("Wrote %s successfully", output_path)
